...
void example_ckks_basics()
{
    ...
    */
    ...
    Plaintext plain_result_true;
    decryptor.decrypt(x2_encrypted, plain_result_true);
    vector<double> result_true;
    encoder.decode(plain_result_true, result_true);
    cout << "    + Computed result ...... Correct." << endl;
    print_vector(result_true, 3, 7);
    ...
    evaluator.transform_from_ntt_inplace(x1_encrypted);
    for (size_t poly_idx = 0; poly_idx < 2; poly_idx++) {
    ...
    for (size_t poly_idx = 0; poly_idx < 2; poly_idx++) {
        ...
        uint64_t *poly_ptr = x1_encrypted.data(poly_idx);
        saveHighBitsToTxt(poly_ptr, N, 35, "/home/qemu/SEAL/SEAL/native/CKKS/flip.txt");
        break;   
    }
    evaluator.transform_to_ntt_inplace(x1_encrypted);
    ...
    x4_encrypted.load(context, ct_stream_1);

    if (!x4_encrypted.is_ntt_form()) {
        evaluator.transform_to_ntt_inplace(x4_encrypted);
        // std::cout<< "1111" << endl;
    }
    ...
    decryptor.decrypt(x4_encrypted, plain_result_1);
